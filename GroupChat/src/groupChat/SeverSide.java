						...
						if (i.getInputFromClient().available() != 0) {
							...
							if (message.split(":")[1].trim().equalsIgnoreCase("QUIT")) {
								...
								if (message.split(":")[1].trim().equalsIgnoreCase("GROUP")) {
									...
									String finalMessage = i.getName() + ": ";

									for (int x = 2; x < tokens.length; x++) {
										finalMessage = finalMessage + tokens[x];
										if (tokens.length > 3) {
									...
									for (Connector j : connections) {
										if (j != i)
											j.getOutput2client().writeUTF(finalMessage);
									}
								} else {
									for (Connector j : connections) {
										String[] tokens = message.split(":");
										if (j.getName().trim().equals(tokens[1].trim())) {
											String finalMessage = "Private Message from " + i.getName() + ":\n   ";
		...
		while (true) {
			try {
				...
				for (Connector i : conections) {
					i.getOutput2client()
							.writeUTF("\n++++++++++++++++++++++++++++++++++++++++++++++\n"
									+ conections.get(conections.size() - 1).getName()
